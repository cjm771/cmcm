...
class EventSource{
	...
	function __construct($action, $root="") {
		require_once($root."php/lib/Jdat.class.php");	
		//initiate
		...
		switch ($this->action){
			...
			case "getMediaFiles":
				...
				foreach ($files as $file){
					...
					//$this->set(self::PROGRESS, "Media: ".$file." Dimensions: ".$dims[0]." x ".$dims[1]);
				}
				$this->set(self::COMPLETE, "DONE!");
				break;
			...
			case "regenerateThumbs":
				//for resizing
				require_once($this->dir."php/lib/ExternalMedia.class.php");
				//get current data file
				$config = Jdat::getSettings($this->dir);
				if (!isset($config->data->mediaFolder))
					$this->set(self::ERROR, "No media Folder Found.");
					...
					foreach ($proj->media as $mediaId=>$media){
						...
						//else if src doesnt exist, check if thumb does
								...
								if ($externalType = ExternalMedia::getExternalMediaType($media->src)){
									...
									if ($exMedia){
										//$this->set(self::PROGRESS, "\n------------\ngrabbing ".$externalType." thumbnail from ".$media->src);
						...
						if (isset($thumbPaths)){
							...
							/*
							$this->set(self::PROGRESS, "\n------------\nmode: ".$thumbPaths['mode']."\nmes:".$thumbData['mes']."\nsoftware:".$thumbData['software']."\nimage_path: ".$thumbData['image_path']."\nthumb_path: ".$thumbData['thumb_path']."\norig Dim:".$thumbData['origWidth']." x ".$thumbData['origHeight']."\nNew Dim:".$thumbData['width']." x ".$thumbData['height']);
							*/	
						}
					}
				}
